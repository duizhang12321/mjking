...
Page({
  data: { rooms: [], me: null },
  onShow() { this.setData({ rooms: [], me: user.getCurrentUser() }); store.listRooms().then(list=>this.setData({ rooms: list })) },
  ...
  createRoom() {
    ...
    wx.showModal({ title: '新建房间', editable: true, placeholderText: '输入房间名称', success: (res) => {
      if (res.confirm && res.content) {
        store.createRoom(res.content, u).then(room=>{ wx.navigateTo({ url: `/pages/room/index?id=${room.id}` }) })
      }
    } })
  },
