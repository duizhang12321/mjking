...
function request(method, path, data){
  return new Promise((resolve, reject)=>{
    ...
    if (typeof path === 'string' && path.indexOf('undefined') >= 0) {
...
function getRoom(id){ if(!id || id === 'undefined'){ return Promise.resolve(null) } return request('GET', `/api/rooms/${id}`).catch(()=> (get(KEYS.rooms).find(r=>r.id===id))) }
